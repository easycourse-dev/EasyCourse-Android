...
public class SignupLoginActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(io.easycourse.www.easycourse.R.layout.activity_signuplogin);
        ...
        if (savedInstanceState == null) {
            if (!currentUser.equals("") && joinedCourses.size() == 0) {
                ...
                fragmentManager.beginTransaction()
                        .replace(io.easycourse.www.easycourse.R.id.activity_signuplogin_container, chooseUniversity).commit();
                return;
            }
            FragmentManager manager = getSupportFragmentManager();
            FragmentTransaction transaction = manager.beginTransaction();

            transaction.replace(io.easycourse.www.easycourse.R.id.activity_signuplogin_container, SignupLogin.newInstance(), "SignupLogin");
            transaction.commit();
        }    }
}
