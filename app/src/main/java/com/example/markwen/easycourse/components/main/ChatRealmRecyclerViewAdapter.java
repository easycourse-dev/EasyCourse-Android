...
public class ChatRealmRecyclerViewAdapter extends RealmBasedRecyclerViewAdapter<Message, RealmViewHolder> {


    private final int INCOMING = 0, OUTGOING = 1;


    public ChatRealmRecyclerViewAdapter(Context context, RealmResults<Message> messages, boolean autoUpdate, boolean animateIdType) {
        super(context, messages, autoUpdate, animateIdType);
    }

    private class IncomingChatViewHolder extends RealmViewHolder {
    ...
    private class OutgoingChatViewHolder extends RealmViewHolder {
    ...
    public RealmViewHolder onCreateRealmViewHolder(ViewGroup viewGroup, int viewType) {
        RealmViewHolder viewHolder;
        LayoutInflater inflater = LayoutInflater.from(viewGroup.getContext());

        switch (viewType) {
            case OUTGOING:
                View outgoingView = inflater.inflate(R.layout.chat_cell_outgoing_text, viewGroup, false);
                viewHolder = new ChatRealmRecyclerViewAdapter.OutgoingChatViewHolder(outgoingView);
                break;
            default:
                View incomingView = inflater.inflate(R.layout.chat_cell_incoming_text, viewGroup, false);
                viewHolder = new ChatRealmRecyclerViewAdapter.IncomingChatViewHolder(incomingView);
                break;
        }
        return viewHolder;
    }
    ...
    public void onBindRealmViewHolder(RealmViewHolder viewHolder, int position) {
        ...
        if (position != 0)
            prevMessage = realmResults.get(position - 1);


        switch (viewHolder.getItemViewType()) {
            case OUTGOING:
                final ChatRealmRecyclerViewAdapter.OutgoingChatViewHolder outgoingViewHolder = (ChatRealmRecyclerViewAdapter.OutgoingChatViewHolder) viewHolder;
            ...
            case INCOMING:
                ChatRealmRecyclerViewAdapter.IncomingChatViewHolder incomingViewHolder = (ChatRealmRecyclerViewAdapter.IncomingChatViewHolder) viewHolder;
    ...
    public int getItemViewType(int i) {
        final Message message = realmResults.get(i);
        if (message.isToUser())
            return OUTGOING;
        return INCOMING;
    }
